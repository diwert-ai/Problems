...
def partition_hoar(array:list,start:int,finish:int) -> int:
    '''
       Алгоритм Хоара разделеия данных массива на две части
    '''
...
def partition_lomuto(array:list, start:int, finish:int) -> int:
    '''
       ...
       больше опорного элемента (pivot) и меньше либо равных опорному элемента.
       Разделение осуществлеяется внутри исходного массива - без использования 
...
def hoar_sort_recursive_lomuto(array:list,start:int,finish:int):
    ...
    #рекурсивно сортируем подмасивы оставляя на месте элемент c индексом bound
...
def test4():
    ...
    for test in tests:
        print(test)
        bound = partition_hoar(test,0,len(test)-1)
        print(test)
        print(bound)
